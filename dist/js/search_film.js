...
getPagelistFromSearchKeyWord = (list, keyword) => {
  let result = [];
  let key = keyword.toLowerCase();

  result = list.filter((val, idx) => val.ten.toLowerCase().indexOf(key) > -1);
...
function getItems(page, keyword) {
  $(".header-title").html(`
    <span class="title">
      Tìm Kiếm
      <p>Nội dung tìm kiếm: ${keyword}</p>
    </span>
  `);
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      let max = limit * page;
      let dataCheck = getPagelistFromSearchKeyWord(data, keyword);
...
$(document).ready(function () {
  ...
  keywordSearching = getUrlParameter("inputsearch");
  RenderProfile();
  RenderMenuOption("all");
  RenderLogoBrand();
  getItems(currentPage, keywordSearching);
