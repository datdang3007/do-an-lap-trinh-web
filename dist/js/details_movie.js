  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      let pathText = `
        ...
        <span>&nbsp;Xem phim > Phim ${filmData.theloai} >&nbsp;</span>
        <span style="color: #fff;">${filmData.ten}</span>
      `;
      ...
      let groupPathToFilm = `
        ...
        <div id="btnChangePathToFilm" data-id="${filmData.id}" data-series="${filmData.phimle}">
            <i class="fa-solid fa-play"></i>
        </div>
        <img class="small-screen" src="${filmData.image}" alt=""></img>
        <div class="group-text-btn">
            <span class="name">${filmData.ten}</span>
            <span class="author">Đạo diễn: ${filmData.daodien}</span>
            <div class="group-btn-trailer-film flex-center">
                <div id="btn-trailer">
                    <i class="fa-brands fa-youtube"></i>
                    Trailer
                </div>
                <div id="btn-film" data-id="${filmData.id}" data-series="${filmData.phimle}">
                    <i class="fa-solid fa-play"></i>
                    Xem phim
                </div>
            </div>
      ...
      social += `
          ...
          <div class="details-film">
              <ul class="details-list">
                  <li>
                      <span class="title">Đặc Biệt:</span>
                      <span class="content" style="color: #d1322c;">${filmData.dacbiet}</span>
                  </li>
                  <li>
                      <span class="title">Năm Phát Hành:</span>
                      <span class="content">${filmData.nam}</span>
                  </li>
                  <li>
                      <span class="title">Quốc Gia:</span>
                      <span class="content">${filmData.quocgia}</span>
                  </li>
                  <li>
                      <span class="title">Thể Loại:</span>
                      <span class="content">${filmData.theloai}</span>
                  </li>
                  <li>
                      <span class="title">Đạo Diễn:</span>
                      <span class="content">${filmData.daodien}</span>
                  </li>
                  <li>
                      <span class="title">Thời Lượng:</span>
                      <span class="content">${filmData.thoiluong}</span>
                  </li>
                  <li style="width: 100%;">
                      <span class="title">Diễn Viên:</span>
                      <span class="content">${filmData.dienvien}</span>
                  </li>
              </ul>
          </div>

          <span class="title-of-content">Nội dung phim</span>
          <div class="group-movie-content"><b>${filmData.ten}</b> ${filmData.noidung}</div>
      
          <span class="title-of-content">Tags</span>
          <div class="group-tags flex-center">
      `;

      for (const [key, value] of Object.entries(filmData.tags)) {
        social += `
          <div class="tag">
            <div class="tag-box">
                ${value}
      ...
      $(".path-text").html(pathText);
      $(".group-path-to-film").html(groupPathToFilm);
...
$(document).ready(function () {
  ...
  setTimeout(() => {
    ...
    $("#btnChangePathToFilm").click(() => {
      let id = $("#btnChangePathToFilm").data().id;
      let series = $("#btn-film").data().series;
      changeToMovie(id, series, 1);
    });

    $("#btn-film").click(() => {
      let id = $("#btn-film").data().id;
      let series = $("#btn-film").data().series;
      changeToMovie(id, series, 1);
    });
